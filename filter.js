...
const button = document.querySelector(".btn_view")
...
function loadPosts(){
    ...
    filteredPosts.slice(0,maxDisplayLimit)
...
function filterPosts(){
    const searchFilter = post =>
        [post.title, post.summary, post.user.name[0].firstName,
            post.user.name[1].lastName, post.meta.tags.map(t => t).join("")]
            .map(text => text).join("").toLowerCase().indexOf(search.value.toLowerCase()) !== -1;
...
fetchPosts();
button.addEventListener("click", viewMorePost)
